...
class PointTransformerConv(MessagePassing):
    ...
    def message(self, x_j: Tensor, pos_i: Tensor, pos_j: Tensor,
        ...
        msg = self.normalized_point_pair_features(pos_i, pos_j, normal_i, normal_j, radius_i)   # (61035,6) -> (1373,6)???
        # print("msg :", msg)
        # from torch_geometric.nn import global_max_pool, radius, global_mean_pool, knn
        # out = global_max_pool(msg, pool_batch)
        # print(out[9])
        delta = self.pos_nn(msg)     # (61035,6) -> (61035,128)
        alpha = alpha_i - alpha_j + delta    #
        if self.attn_nn is not None:
            alpha = self.attn_nn(alpha)     # MLP
